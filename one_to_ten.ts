{
  ...
  const multipleAorB = (number: number, a: number, b: number): number => {
    //! number보다 작은 수 중에서 a 와 b 의 공배수를 result에 플러스 할당하여 반환
    let result = 0;
  ...
  const sumEvenNumberFibonacci = (number: number): number => {
    //! 피보나치 수열을 배열에 전개하며, 그 중 짝수인 수만 result에 플러스 할당하여 반환
    let result = 2;
    let lists = [1, 2];
    while (true) {
  ...
  // 3
  const lagestPrimeNumber = (number: number): number => {
    //! loop를 통해 소수인지 판별하여 가장 큰 소인수 반환
    let result: number = 0;
  ...
  const lagestPalindrome = (): number | undefined => {
    //! 세자리수를 곱해 만들 수 있는 가장 큰 팰린드롬을 반환
    let result: number = 0;
    for (let j = 900; j < 1000; j++) {
      for (let i = 900; i < 1000; i++) {
        //! 가장 큰 대칭수를 찾아야 하니, 900대 부터 loop시작
        if (
          String(i * j) ===
          String(i * j)
            .split("")
  ...
  const lowestCommonMultipleUnderInputNumber = (number: number): any => {
    ...
    const largestPower = (count: number, number: number): number => {
      let powed: number;
      let e = 2;
      let largestCount = count;
      while ((powed = Math.pow(count, e)) <= number) {
        largestCount = powed;
        e += 1;
      }
      return largestCount;
    ...
    const isPrime = (count: number): boolean => {
      let i: number;
      let number = Math.ceil(Math.sqrt(count));
    ...
    for (i = 3; i <= number; i += 2) {
      if (isPrime(i)) {
        count *= largestPower(i, number);
      }
    }

    return count * largestPower(2, number);
  };
  console.log(`5번: ${lowestCommonMultipleUnderInputNumber(20)}`);

  // 6
  const squaredSumMinusSumOfSquare = (maxNumber: number): number => {
    //! 각 수들의 제곱의 합과 합의 제곱을 구해 차이를 반환
    if (maxNumber === 0) {
  ...
  const primeNumberOfOrder = (order: number): number => {
    //! primeNumbers 배열에 소수를 차례로 push 후, order번째 값 반환
  ...
  const lasgestNumberInThousandDigits = (
    stringifyNumbers: Array<string>
  ): number => {
    let result: number = 0;
    for (let i = 0; i < stringifyNumbers.length; i++) {
      for (let j = 0; j < stringifyNumbers[i].length; j++) {
        ...
        if (i !== stringifyNumbers.length - 1) {
          //! 연속된 5개의 수의 곱셈, 다음 수가 없을 시 배열의 다음 인덱스 첫번째 수 부터 조회
          if (!stringifyNumbers[i][j + 1]) {
            multiplication =
              Number(stringifyNumbers[i][j]) *
              Number(stringifyNumbers[i + 1][0]) *
              Number(stringifyNumbers[i + 1][1]) *
              Number(stringifyNumbers[i + 1][2]) *
              Number(stringifyNumbers[i + 1][3]);
          } else if (!stringifyNumbers[i][j + 2]) {
            multiplication =
              Number(stringifyNumbers[i][j]) *
              Number(stringifyNumbers[i][j + 1]) *
              Number(stringifyNumbers[i + 1][0]) *
              Number(stringifyNumbers[i + 1][1]) *
              Number(stringifyNumbers[i + 1][2]);
          } else if (!stringifyNumbers[i][j + 2]) {
            ...
            multiplication =
              ...
              Number(stringifyNumbers[i][j + 2]) *
              Number(stringifyNumbers[i][j + 3]) *
              Number(stringifyNumbers[i][j + 4]);
          }
        } else {
          if (!stringifyNumbers[i][j + 1]) {
            //! 배열의 다음 인덱스 첫번째 수 조차 없을 때 break
            break;
          }
        }
  ...
  // 9
  const multiplicationOfPythagorasNumbers = (
      ...
      for (b = 1; b <= sumOfThreeNumbers / 2; b++) {
        for (c = 1; c <= sumOfThreeNumbers / 2; c++) {
          if (a + b + c === 1000 && a * a + b * b === c * c) {
            //! 3중 loop로 세 숫자의 합이 1000인 조건과 피타고라스 정리를 만족하는지 확인 후 반환
            return a * b * c;
          }
        }
      }
  ...
  const sumOfPrimeNumbersUnderInputNumber = (number: number): number => {
    //! 하나씩 조회하여 소수인지 판별 후, 배열에 담아 전부 더해준 후 반환
